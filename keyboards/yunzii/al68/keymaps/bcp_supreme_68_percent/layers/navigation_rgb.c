void navigation_rgb_indicators(void) {
    ...
    uint8_t dark_blue_r = 0;
    uint8_t dark_blue_g = 0;
    uint8_t dark_blue_b = (brightness * 64) / 255;

    uint8_t dark_purple_r = (brightness * 64) / 255;
    uint8_t dark_purple_g = 0;
    uint8_t dark_purple_b = (brightness * 64) / 255;

    uint8_t orange_r = (brightness * 200) / 255;
    uint8_t orange_g = (brightness * 100) / 255;
    uint8_t orange_b = 0;
        ...
        for (uint8_t column = 0; column < MATRIX_COLS; column++) {
            ...
            if (index != NO_LED) {
                ...
                if (keycode != KC_TRNS && keycode != KC_NO) {
                    ...
                    } else if (keycode == MACRO_FOCUS_WINDOW_LEFT || keycode == MACRO_FOCUS_WINDOW_RIGHT) {
                        rgb_matrix_set_color(index, dark_blue_r, dark_blue_g, dark_blue_b);
                    } else if (keycode == MACRO_FOCUS_SPACE_LEFT || keycode == MACRO_FOCUS_SPACE_RIGHT) {
                        rgb_matrix_set_color(index, dark_purple_r, dark_purple_g, dark_purple_b);
                    } else if (keycode == MACRO_UNDO) {
                        rgb_matrix_set_color(index, dark_purple_r, dark_purple_g, dark_purple_b);
                    } else if (keycode == MACRO_CUT) {
                        rgb_matrix_set_color(index, red_r, red_g, red_b);
                    } else if (keycode == MACRO_COPY) {
                        rgb_matrix_set_color(index, blue_r, blue_g, blue_b);
                    } else if (keycode == MACRO_PASTE) {
                        rgb_matrix_set_color(index, green_r, green_g, green_b);
                    } else if (keycode == MACRO_GO_BACK || keycode == MACRO_GO_FORWARD) {
                        rgb_matrix_set_color(index, orange_r, orange_g, orange_b);
