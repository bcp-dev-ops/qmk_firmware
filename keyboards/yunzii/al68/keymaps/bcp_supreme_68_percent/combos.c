enum combo_events {
    ...
    COMBO_ALPHA_QWERTY_ESC_1,
};

// ---- Combo definition ----
const uint16_t PROGMEM dragscroll_combo[] = {
    MS_BTN1,
    LT(L_FUNCTIONAL, MS_BTN2),
    COMBO_END
};

const uint16_t PROGMEM esc_1_combo[] = {
    MS_BTN1,
    KC_1,
    COMBO_END
};
...
combo_t key_combos[] = {
    ...
    [COMBO_ALPHA_QWERTY_ESC_1] = COMBO_ACTION(esc_1_combo),
};
...
bool combo_should_trigger(uint16_t combo_index, combo_t *combo, uint16_t keycode, keyrecord_t *record) {
    switch (combo_index) {
        case COMBO_ALPHA_QWERTY_DRAGSCROLL:
            return layer_state_is(L_ALPHA_QWERTY);
        case COMBO_ALPHA_QWERTY_ESC_1:
            return layer_state_is(L_ALPHA_QWERTY);
    }
    return true;
}
